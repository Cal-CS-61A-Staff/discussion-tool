...
def extract_predict_examples(prompt, starter_code, expected_output):
    ...
    Fallback: content whose prompt shows a >>> call with no output
    following it (the tree worksheet's prose style) but that does have a
    single expected_output value authored separately — synthesize one
    example from the last >>> line in the prompt plus that value.
    """
    ...
    if expected_output:
        ...
        call = calls[-1].strip() if calls else "this code"
...
def _examples_from_docstrings(starter_code):
    ...
    for node in ast.walk(tree):
        ...
        if not docstring:
            continue
        try:
            for ex in parser.get_examples(docstring):
                if ex.want.strip():
                    examples.append({"call": ex.source.strip(), "expected": ex.want.strip()})
        except ValueError:
            continue
    return examples
